...
{  
    ...
    var countDisplay;
    var isInline = options.container != undefined;
    var isPlaying = false;
...
//                on the keyboard, specify the Next button. If no button should be display, leave blank.
    ...
    function createPopup()
    {
        ...
        popup.appendTo(isInline ? options.container : $(parent.document.body));
    ...
    {
        var widthVal = isInline ? options.container.width() : $(parent.window).width();
        var heightVal = isInline ? options.container.height() : $(parent.window).height();
        var returnValue = {
            height : heightVal,
            width : widthVal,
